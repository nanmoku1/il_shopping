        ...
        <div class="row">
            <div class="col-md-4 mb-3">
                <select class="custom-select" id="product_category_id" name="product_category_id">
                    ...
                    <option value="{{ $product_category->id }}" {{ $request->productCategoryId() == $product_category->id ? "selected":"" }}>{{ $product_category->name }}</option>
                    @endforeach
                </select>
            </div>
                    ...
                    <div class="input-group-append">
                        <div class="input-group-text">
                            ...
                            <div class="form-check form-check-inline mr-0">
                                <input class="form-check-input" type="radio" name="price_compare" id="price-compare-lteq" value="lteq" {{ $request->priceCompare() === "lteq" ? "checked":"" }}>
        ...
        <div class="row">
            <div class="col-md-4 mb-3">
                <select class="custom-select" name="sort_column">
                    ...
                    <option value="product_category" {{ $request->sortColumn() === "product_category" ? "selected":"" }}>並び替え: 商品カテゴリ</option>
                    <option value="name" {{ $request->sortColumn() === "name" ? "selected":"" }}>並び替え: 名称</option>
                    <option value="price" {{ $request->sortColumn() === "price" ? "selected":"" }}>並び替え: 価格</option>
                </select>
            </div>
            <div class="col-md-4 mb-3">
                <select class="custom-select" name="sort_direction">
                    <option value="asc" {{ $request->sortDirection() === "asc" ? "selected":"" }}>並び替え方向: 昇順</option>
                    <option value="desc" {{ $request->sortDirection() === "desc" ? "selected":"" }}>並び替え方向: 降順</option>
                </select>
            </div>
            <div class="col-md-2 mb-3">
                <select class="custom-select" name="page_unit">
                    <option value="10">表示: 10件</option>
                    <option value="20" {{ $request->pageUnit() == 20 ? "selected":"" }}>表示: 20件</option>
                    <option value="50" {{ $request->pageUnit() == 50 ? "selected":"" }}>表示: 50件</option>
                    <option value="100" {{ $request->pageUnit() == 100 ? "selected":"" }}>表示: 100件</option>
                </select>
            </div>
